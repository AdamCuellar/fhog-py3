...
gradientHist(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O!O!O!iiiif",
        &PyArray_Type, &arg1, 
        &PyArray_Type, &arg2, 
        &PyArray_Type, &arg3,
        &binSize,
    ...
    if( useHog==0 ) {
        gradHist((float *) M, (float *) O, (float *) H, h, w, binSize, nOrients, softBin, full );
    } else if(useHog==1) {
        hog((float *) M, (float *) O, (float *) H, h, w, binSize, nOrients, softBin, full, clipHog );
    } else {
